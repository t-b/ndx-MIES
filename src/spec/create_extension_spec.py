...
def main():
    ...
    stimset_parameter_segments = NWBDatasetSpec(
        doc="Stimulus set parameters for the full set. "
    ...
    stimset_parameter_textual = NWBDatasetSpec(
        doc="Textual part of the stimulus set parameter waves for recreating the stimset in MIES. "
    ...
    stimset_parameter_numerical = NWBDatasetSpec(
        doc="Numerical part of the stimulus set parameter waves for recreating the stimset in MIES. "
    ...
    stimset_waveform = NWBDatasetSpec(
        doc="Stimulus set waveform data. This is only present if not all three parameter waves could be found "
    ...
    stimset_referenced_waveform = NWBDatasetSpec(
        doc="Additional stimulus set waveform data. Some epoch types for stimulus sets allow to include arbitrary waveform data. "
    ...
    stimset_referenced_folder = NWBGroupSpec(
        doc="Folder",
    ...
    stimset_referenced = NWBGroupSpec(
        doc="Additional stimulus set waveform data is store here in tree structure.",
        neurodata_type_def="StimulusSetReferenced",
        groups=[NWBGroupSpec(doc="Folder", neurodata_type_inc="StimulusSetReferencedFolder", quantity="*")],
    )

    # END custom wave handling

    stimsets = NWBGroupSpec(
    ...
    textual_keys = NWBDatasetSpec(
        doc="Textual labnotebook keys: First row is the name, second row is the unit and third row is the tolerance. "
    ...
    textual_values = NWBDatasetSpec(
        doc="Textual labnotebook values: Rows are the "
    ...
    numerical_keys = NWBDatasetSpec(
        doc="Numerical labnotebook keys: First row is the name, second row is the unit and third row is the tolerance. "
        "Columns are the running index. "
        "See also https://alleninstitute.github.io/MIES/labnotebook-docs.html.",
    ...
    numerical_values = NWBDatasetSpec(
        doc="Numerical labnotebook values: Rows are the "
        "running index, Columns hold the different entry names, Layers (up to nine) hold "
        "the headstage dependent data in the first 8 and the headstage independent data in the 9th layer. "
        "See also https://alleninstitute.github.io/MIES/labnotebook-docs.html.",
    ...
    labnotebook_device = NWBGroupSpec(
    ...
    labnotebook = NWBGroupSpec(
        doc="Labnotebooks",
        neurodata_type_def="LabNotebook",
        groups=[NWBGroupSpec(doc="Device for the labnotebooks", neurodata_type_inc="LabNotebookDevice")],
    )

    # Since 05cb3ffe (NWB: Export single test pulses as well on interactive use, 2017-11-07)

    stored_testpulses = NWBDatasetSpec(
        doc="Raw AD testpulse data", neurodata_type_def="TestpulseRawData", neurodata_type_inc="NWBData", shape=(None,)
    )
    ...
    tpstorage = NWBDatasetSpec(
        doc="Metadata about the Testpulse: Rows are the running index, Columns are active AD channels "
    ...
    testpulse_device = NWBGroupSpec(
        ...
        datasets=[
            ...
            NWBDatasetSpec(doc="Raw AD testpulse data", neurodata_type_inc="TestpulseRawData", quantity="*"),
        ],
    )

    testpulse = NWBGroupSpec(
        doc="Testpulse data",
        neurodata_type_def="Testpulse",
        groups=[NWBGroupSpec(doc="Device for the testpulse data", neurodata_type_inc="TestpulseDevice")],
    )

    user_comment_device_string = NWBDatasetSpec(
        "device specific user text notes", neurodata_type_def="UserCommentString", neurodata_type_inc="NWBData"
    )

    user_comment_device = NWBGroupSpec(
        "Device for the user text notes",
        neurodata_type_def="UserCommentDevice",
        neurodata_type_inc="Device",
        datasets=[NWBDatasetSpec("device specific user text notes", neurodata_type_inc="UserCommentString")],
    )

    user_comment = NWBGroupSpec(
        doc="Free form text notes from the experimenter",
        neurodata_type_def="UserComment",
        groups=[NWBGroupSpec("Device for the user text notes", neurodata_type_inc="UserCommentDevice")],
    )

    generated_by = NWBDatasetSpec(
        doc="Software provenance information as key " "(first column) value (second column) pairs.",
        neurodata_type_def="GeneratedBy",
        neurodata_type_inc="NWBData",
        shape=(None, 2),
    )

    mies_general = NWBGroupSpec(
        ...
        groups=[
            NWBGroupSpec(
                doc="Free form text notes from the experimenter", neurodata_type_inc="UserComment", quantity="?"
            ),
            NWBGroupSpec(doc="Testpulse data", neurodata_type_inc="Testpulse", quantity="?"),
            NWBGroupSpec(doc="Labnotebooks", neurodata_type_inc="LabNotebook"),
            NWBGroupSpec(
                doc="Stimulus Sets: Parameter waves, referenced custom waves and third-party stimsets",
    ...
    new_data_types = [
        ...
        stimsets,
        stimset_waveform,
        stimset_parameter_numerical,
        stimset_parameter_textual,
        stimset_parameter_segments,
        stimset_referenced,
        stimset_referenced_folder,
        stimset_referenced_waveform,
        labnotebook_device,
        mies_general,
    ]
